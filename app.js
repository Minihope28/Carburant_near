...
```
  ...
  let tags = "";
  if (isCheapest) {
    tags += `<span class="tag tag-green"><i class="fas fa-check"></i> Meilleur prix</span>`;
  }
  if (isClosest) {
    tags += `<span class="tag tag-blue"><i class="fas fa-location-arrow"></i> Plus proche</span>`;
  }
  if (!isCheapest && !isClosest && CONFIG.sortBy === "both") {
    tags += `<span class="tag tag-amber"><i class="fas fa-balance-scale"></i> Bon compromis</span>`;
  }
    ...
    <div class="station-card" style="animation-delay:${index * 0.04}s">
      <div class="card-body">
        <div class="card-main">
          ...
          <div class="card-meta">
            <span class="card-meta-item">
              ...
              <span class="highlight">${station.distance.toFixed(1)} km</span>
            </span>
            <span class="card-meta-item">
        ...
        <div class="card-price">
          <div class="price-bubble ${priceClass}">${station.price.toFixed(3)}€</div>
          ${isCheapest
...
document.getElementById(“stationsList”).innerHTML = `<div class="error-state"> <div class="empty-icon-wrap" style="background:var(--red-dim);color:var(--red);margin:0 auto 16px;"> <i class="fas fa-exclamation-triangle"></i> </div> <p class="empty-title" style="color:var(--red)">${escapeHtml(message)}</p> <button onclick="refreshData()" style="margin-top:16px;background:var(--amber);color:#000;border:none;padding:10px 20px;border-radius:8px;font-weight:700;font-size:13px;cursor:pointer;"> Réessayer </button> </div>`;
}

function showEmpty(message) {
document.getElementById(“stationsList”).innerHTML = `<div class="empty-state"> <div class="empty-icon-wrap"><i class="fas fa-search"></i></div> <p class="empty-title">${escapeHtml(message)}</p> <p class="empty-sub">Essaie d'augmenter le rayon ou de changer de carburant</p> </div>`;
}
