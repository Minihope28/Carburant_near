...
async function searchStations(lat, lon) {
  document.getElementById("stationsList").innerHTML = `
    <div class="glass-card empty-state">
      ...
      <p class="empty-subtitle">Cela peut prendre quelques secondes</p>
    </div>
  `;
...
function updateUI() {
  ...
  stationsList.innerHTML = stations
    ...
    .map((station, index) => {
      ...
      return `
        <div class="glass-card station-card animate-slide-up" style="animation-delay:${index * 0.04}s">
          <div class="p-5">
            <div class="flex items-start justify-between gap-4">
              <div class="flex-1 min-w-0">
                <div class="flex items-center flex-wrap gap-2 mb-2">
                  <h3 class="font-bold text-base text-white truncate">${escapeHtml(station.brand)}</h3>
                  ${badges}
                </div>
              ...
              <div class="text-right flex flex-col items-end">
                ...
                ${
                  !isCheapest
                    ? `<div class="text-xs text-slate-400 mt-2">+${((station.price - cheapestPrice) * 100).toFixed(1)} c</div>`
                    : `<div class="text-xs text-green-400 font-semibold mt-2">Top prix</div>`
                }
              </div>
            </div>
            ...
            <div class="mt-4 pt-4 border-t border-white/5 flex items-center justify-between">
              <a
                ...
                class="text-blue-400 text-sm hover:text-blue-300 inline-flex items-center gap-2"
              >
...
function showError(message) {
  document.getElementById("stationsList").innerHTML = `
    <div class="glass-card error-card p-8 text-center">
      <i class="fas fa-exclamation-triangle text-red-400 text-4xl mb-3"></i>
      <p class="text-red-200 mb-3">${escapeHtml(message)}</p>
      <button onclick="refreshData()" class="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-sm font-medium">
        Réessayer
      </button>
    </div>
  `;
...
function showEmpty(message) {
  document.getElementById("stationsList").innerHTML = `
    <div class="glass-card empty-state">
      ...
      <p class="empty-subtitle">Essaie d’augmenter le rayon ou de changer de carburant</p>
    </div>
  `;
}
