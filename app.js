    ...
    if (stationsList) {
      stationsList.innerHTML = `
        <article class="empty-state">
          ...
          <h3>Recherchez une ville au Maroc</h3>
          <p>Exemples : Casablanca, Rabat, Tanger, Marrakech, Agadir, Fès.</p>
        </article>
      `;
    }
...
async function searchMoroccoCity(cityQuery) {
  const stationsList = document.getElementById("stationsList");
  ...
  stationsList.innerHTML = `
    <article class="loading-state">
      ...
      <p>Récupération des stations et prix estimatifs</p>
    </article>
  `;
  ...
  try {
    ...
    const url = `${CONFIG.moroccoApiUrl}?city=${encodeURIComponent(cityQuery)}`;
    const res = await fetch(url);
    ...
    showToast("Ville Maroc mise à jour", "success");
  } catch (err) {
    ...
    stationsList.innerHTML = `
      <article class="error-state">
        ...
        <h3>Impossible de charger la ville pour le moment</h3>
        <p>La vue Maroc est prête, mais le worker doit encore brancher les sources externes.</p>
      </article>
    `;

    showToast("Worker Maroc non branché ou indisponible", "error");
  }
}
...
function renderMoroccoOverview(data) {
  ...
  if (gasEl) gasEl.textContent = data.country_prices?.gasoline || "—";
  if (dieselEl) dieselEl.textContent = data.country_prices?.diesel || "—";
  if (sourceNote) sourceNote.textContent = data.source_label || "Données estimatives / source tierce";

  const brandNames = Object.keys(data.brand_prices || {});
  if (brandsEl) {
    if (brandNames.length) {
      brandsEl.innerHTML = brandNames
        .map((brand) => `<span class="brand-pill">${escapeHtml(brand)}</span>`)
        .join("");
    } else {
      brandsEl.innerHTML = `
  ...
  if (stationCount) {
    ...
    stationCount.textContent = count ? `${count} station${count > 1 ? "s" : ""}` : "Mode bêta";
  }

  if (fuelValue) fuelValue.textContent = "Essence / Diesel";
  if (bestPriceEl) bestPriceEl.textContent = data.country_prices?.gasoline || "—";
...
function renderMoroccoStations(data) {
  ...
  const brandPrices = data.brand_prices || {};
  ...
  if (!stations.length) {
    stationsList.innerHTML = `
      <article class="empty-state">
        ...
        <p>La ville a été reconnue, mais aucune station n’a encore été renvoyée par le worker.</p>
      </article>
    `;
    return;
  }
  ...
  stationsList.innerHTML = stations
    .slice(0, 20)
    .map((s, index) => {
      const brand = s.brand || s.name || "Station";
      const brandInfo = brandPrices[brand] || null;

      const gasoline = brandInfo?.gasoline || data.country_prices?.gasoline || "Estimatif";
      const diesel = brandInfo?.diesel || data.country_prices?.diesel || "Estimatif";
        ...
        <article class="station-card" style="animation-delay:${index * 0.04}s">
          <div class="card-body">
            <div class="card-main">
              ...
              <div class="card-meta">
                <span class="card-meta-item">
                  <i class="fa-solid fa-gas-pump"></i>
                  Essence ${escapeHtml(gasoline)}
                </span>
                <span class="card-meta-item">
                  <i class="fa-solid fa-oil-can"></i>
                  Diesel ${escapeHtml(diesel)}
                </span>
              </div>
            </div>
            ...
            <div class="card-price">
              ...
              <span class="price-delta">Source tierce</span>
            </div>
          </div>
...
function refreshData() {
  ...
  if (CONFIG.country === "MA") {
    const city = document.getElementById("addressInput")?.value?.trim() || "Casablanca";
    searchMoroccoCity(city);
    return;
  }
